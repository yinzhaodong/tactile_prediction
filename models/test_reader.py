...
def create_test_plots_aligned(experiment_to_test):
    ...
    print(trial_groundtruth_data[0][0])
    ...
    line_1 = ax1.plot([None for i in range(10)] + [i for i in predicted_taxel_t10], alpha=1.0, c="b", label="Pred_t10")
    line_3 = ax1.plot(groundtruth_taxle, alpha=1.0, c="r", label="Gt+10")
    ...
    plt.title("ALIGNED " + model_name + " model taxel " + str(index) + ". Test " + str(test_trial) + ". Seen at training: " + meta[1][2] + ". Object: " + meta[1][0])
...
def create_test_plots(experiment_to_test):
    ...
    print(trial_predicted_data_t1[0][0])
    print(trial_groundtruth_data[0][0])
    ...
    for index, axi in enumerate(ax.flat):
        ...
        axi.set_xlabel('time step')
        axi.set_ylabel('tactile reading')
        line_5 = axi.plot([i for i in predicted_taxel_t1], alpha=1.0, c="orange", label="Pred_t1")
        line_2 = axi.plot([i for i in predicted_taxel_t5], alpha=1.0, c="g", label="Pred_t5")
        line_1 = axi.plot([i for i in predicted_taxel_t10], alpha=1.0, c="b", label="Pred_t10")
        line_3 = axi.plot(groundtruth_taxle, alpha=1.0, c="r", label="Gt")
        axi.tick_params(axis='y')

        lines = line_1 + line_2 + line_3 + line_5
        labs = [l.get_label() for l in lines]

        # fig.tight_layout()  # otherwise the right y-label is slightly clipped
        fig.subplots_adjust(top=0.90)
        fig.legend(lines, labs, loc="upper right")

        axi.grid(which='minor')
        axi.grid(which='major')
        # axi.title(model_name + " model taxel " + str(index) + ". Test " + str(test_trial) + ". Seen at training: " + meta[1][2] + ". Object: " + meta[1][0])
        # plt.savefig(plot_save_dir + '/test_plot_taxel_' + str(index) + '.png', dpi=300)
    plt.show()
    ...
    for index, axi in enumerate(ax.flat):
        ...
        predicted_taxel_t10 = []
        for k in range(len(trial_predicted_data_t10)):
            predicted_taxel_t1.append(trial_predicted_data_t1[k][16+index])
            predicted_taxel_t5.append(trial_predicted_data_t5[k][16+index])
            predicted_taxel_t10.append(trial_predicted_data_t10[k][16+index])
            groundtruth_taxle.append(trial_groundtruth_data[k][16+index])
    ...
    for index, axi in enumerate(ax.flat):
        ...
        predicted_taxel_t10 = []
        for k in range(len(trial_predicted_data_t10)):
            predicted_taxel_t1.append(trial_predicted_data_t1[k][32 + index])
            predicted_taxel_t5.append(trial_predicted_data_t5[k][32 + index])
            predicted_taxel_t10.append(trial_predicted_data_t10[k][32 + index])
            groundtruth_taxle.append(trial_groundtruth_data[k][32 + index])
...
for test_trial in range(0, 22):
    create_test_plots(test_trial)
    # create_test_plots_aligned(test_trial)