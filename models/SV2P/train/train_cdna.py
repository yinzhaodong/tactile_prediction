...
class CDNATrainer(train.trainlib.BasicTrainer):
    def __init__(self, in_channels: int, cond_channels: int, n_masks: int,
        ...
        elif dataset_name == 'TactileSingle':
            data_loader = ld.TactileSingle(batch_size=self.batch_size, train_percentage=indices[0], validation_percentage=indices[1])
            self.dataset, self.valid_full_loader = data_loader.load_full_data()
            self.run_stages = 'train', 'valid'
            self.sam_train = torch.utils.data.SubsetRandomSampler(indices[0])
            self.sam_valid = more_sampler.ListSampler(indices[1])
        else:
    ...
    def get_trainloader(self):
        loader = None
        if self.dataset_name == 'TactileSingle':
