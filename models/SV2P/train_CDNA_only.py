    ...
    def run(self):
        ...
        for epoch in progress_bar:
            ...
            for index, batch_features in enumerate(self.train_full_loader):
                ...
                loss, kernloss, maskloss, total_loss = self.SV2P_pass_through(tactile, action)
                break
            # validation:
            for index, batch_features in enumerate(self.valid_full_loader):
                tactile = torch.zeros([20,batch_size,3,64,64]).to(device)  # batch_features[1].permute(1, 0, 4, 3, 2).to(device)
                action = batch_features[0].squeeze(-1).permute(1, 0, 2).to(device)
                loss, kernloss, maskloss, total_loss = self.SV2P_pass_through(tactile, action, validation=True)
                break
            break
            ...
            with torch.no_grad():
                for index, (sample_tactile, sample_action) in enumerate(zip(tactiles[0:-1].squeeze(), actions[1:].squeeze())):
                    ...
                    if index > context_frames-1:
                        predictions_t, hidden, cdna_kerns_t, masks_t = self.net(predictions_t, conditions=tsa, hidden_states=hidden)
                        outputs.append(predictions_t)
                    else:
                        predictions_t, hidden, cdna_kerns_t, masks_t = self.net(sample_tactile, conditions=tsa, hidden_states=hidden)
